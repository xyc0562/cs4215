...
let get_partial_type (t:sPL_type) (args:'b list) =
...
let rec type_infer_x (env:env_type) (e:sPL_expr) : sPL_type option * sPL_expr =
  match e with
    ...
    | Cond (e1,e2,e3) ->
          ...
          let (et1, ea1) = type_infer_x env e1 in
          begin
            match et1 with
            | Some BoolType ->
              let (et2, ea2) = type_infer_x env e2 in
              let (et3, ea3) = type_infer_x env e3 in
                if et2 = et3 then (et2, Cond (ea1, ea2, ea3)) else (None,e)
            | _ -> (None,e)
          end
    | Func (te,args,body) ->
          ...
          begin
            ...
            | Some (env2,teo) ->
              begin
                let nenv = env2@env in
                let (bt,ba) = type_infer_x nenv body in
                match bt with
                | Some t1 ->
                    if t1=teo then (Some te, Func (te,args,ba))
                    else (None,e)
                | _ -> (None,e)
              end
          end
    ...
    | Appln (e1,_,args) ->
          ...
          let rec ptm ft ts =
            match (ft,ts) with
            | (Arrow (ht,tt), (Some t)::ts) -> if ht = t then ptm tt ts else None
            | (rt, []) -> Some rt
            | _ -> None
          in
          begin
...
let trans_exp (e:sPL_expr) : C.sPL_expr  =
  let rec aux e =
    ...
    | Let (ls,t,body) ->
      ...
      begin
        match l with
        | [] -> failwith "Type list cannot be empty"
        | x::xs -> let rec helper acc f l =
          begin
            match xs with
            | [] -> acc
            | x::xs -> f acc x
      ...
      let targs = reduce (fun t targ -> Arrow (t,targ)) (List.map (fun (t,_,_) -> t) ls) in
      let f = Func (Arrow (targs,t), List.map (fun (_,v,_) -> v) ls, body) in
      aux (Appln (f,Some t,List.map (fun (_,_,b) -> b) ls))
  in aux e
